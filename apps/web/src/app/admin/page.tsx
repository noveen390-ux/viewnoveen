...
export default function AdminPage() {
  ...
  const stats = [
    ...
    { icon: Tv, label: 'Active Rooms', value: dashboard?.data?.activeRooms || 0, color: 'text-brand-400' },
      ...
      <div className="max-w-6xl mx-auto p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-surface-400">Manage ViewNoveen platform</p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-5 gap-4 mb-8">
              {stats.map((stat) => (
                <div key={stat.label} className="bg-surface-900/50 border border-surface-800 rounded-xl p-4">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    tab === t.id
                      ...
                      : 'text-surface-400 hover:text-white border-transparent',
                  )}
                >
                  {t.label}
                </button>
            ...
            {tab === 'reports' && (
              <div className="space-y-3">
                {reports?.data?.length ? reports.data.map((report: any) => (
                  <div key={report.id} className="bg-surface-900/50 border border-surface-800 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-white font-medium">{report.category}</p>
                        <p className="text-sm text-surface-400 mt-1">{report.reason}</p>
                        <p className="text-xs text-surface-500 mt-1">Target: {report.targetType}</p>
                      </div>
                      <span className={cn(
                  ...
                  <div className="text-center py-12 text-surface-500">No pending reports</div>
                )}
              </div>
            )}
              ...
              <div>
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    className="w-full bg-surface-800 border border-surface-700 rounded-lg pl-10 pr-4 py-2 text-white placeholder:text-surface-500 focus:outline-none focus:border-brand-500"
                  />
                </div>
                {users?.data?.map((u: any) => (
                  <div key={u.id} className="flex items-center gap-3 bg-surface-900/50 border border-surface-800 rounded-lg p-3 mb-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-surface-400">{u.email} · @{u.username}</p>
                    </div>
                    <span className={cn(
                      'text-xs px-2 py-1 rounded-full',
                      u.status === 'online' ? 'bg-green-600/20 text-green-400' : 'bg-surface-700 text-surface-400',
                    )}>
                      {u.status}
                    </span>
                    <span className={cn(
                      'p-1 rounded',
                      u.isVerified ? 'text-blue-400' : 'text-surface-600',
                    )}>
